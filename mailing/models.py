...
class Mailing(models.Model):
    ...
    sending_time = models.DateTimeField(
        verbose_name="время отправки сообщения", null=True, blank=True
    )
...
class Logs(models.Model):
    message = models.TextField(
        verbose_name="сообщение", max_length=1024, null=True, blank=True
    )
    sending_time = models.DateTimeField(
        verbose_name="время отправки сообщения", null=True, blank=True
    )
    recepient = models.TextField(
        verbose_name="получатель", max_length=1024, null=True, blank=True
    )
    server_response = models.TextField(
        verbose_name="ответ сервера", max_length=1024, null=True, blank=True
    )
